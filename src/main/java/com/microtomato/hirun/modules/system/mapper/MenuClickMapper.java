...
public interface MenuClickMapper extends BaseMapper<MenuClick> {
    ...
    @Select("select m.menu_id, m.title, m.menu_url, m.ico_url from sys_menu m, sys_menu_click c where m.menu_id = c.menu_id and c.user_id = #{userId} order by c.clicks desc LIMIT 8")
