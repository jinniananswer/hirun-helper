...
public class DecoratorServiceImpl extends ServiceImpl<DecoratorMapper, Decorator> implements IDecoratorService {
    ...
    @Override
    public IPage<DecoratorServiceDTO> queryDecoratorInfo(String name, String identityNo, String decoratorType, int current, int size) {
        LambdaQueryWrapper<Decorator> lambdaQueryWrapper = Wrappers.lambdaQuery();
        lambdaQueryWrapper.like(StringUtils.isNotEmpty(name), Decorator::getName, name)
        ...
        List<Decorator> decorators = decoratorPages.getRecords();
        List<DecoratorServiceDTO> decoratorServices = new ArrayList<>();
        decorators.forEach(d -> {
            DecoratorServiceDTO decoratorService = new DecoratorServiceDTO();
            BeanUtils.copyProperties(d, decoratorService);
            decoratorServices.add(decoratorService);
        });

        IPage<DecoratorServiceDTO> decorator = new Page<>(page.getCurrent(), page.getSize());
        decorator.setTotal(decoratorPages.getTotal());
        decorator.setRecords(decoratorServices);
