require(['vue', 'ELEMENT', 'ajax', 'vxe-table', 'vueselect', 'org-orgtree', 'house-select', 'util', 'cust-info', 'order-info', 'order-worker', 'order-search-employee', 'order-selectemployee', 'order-selectdecorator'], function (Vue, element, ajax, table, vueSelect, orgTree, houseSelect, util, custInfo, orderInfo, orderWorker, orderSearchEmployee, orderSelectEmployee, orderSelectDecorator) {
    ...
    let vm = new Vue({
            ...
            methods: {
                ...
                queryVoucherSupplyInfo: function () {
                    let that = this;
                    ajax.get('api/finance/finance-voucher/queryVoucherSupplyInfo', this.queryCondForMaterial, function (responseData) {
                ...
                auditForSupplyPass: function () {
                    let data = this.$refs.materialTable.getCheckboxRecords();
                    if (data == null || data.length <= 0) {
                        this.$message.error('没有选中任何记录，无法提交');
                        return;
                    }
                    ajax.post('api/finance/finance-voucher/auditForSupplyPass', this.$refs.materialTable.getCheckboxRecords());
                },
                auditForSupplyReject: function () {
                    let data = this.$refs.materialTable.getCheckboxRecords();
                    if (data == null || data.length <= 0) {
                        this.$message.error('没有选中任何记录，无法提交');
                        return;
                    }
                    ajax.post('api/finance/finance-voucher/auditForSupplyReject', this.$refs.materialTable.getCheckboxRecords());
                },
