layui.extend({
    ...
    var createEmployeePenalty = {
        init: function () {

            layui.select.init('penaltyType', 'PENALTY_TYPE', '1', false);
            ...
            form.on('submit(add-employeePenalty-submit)', function (data) {
                var field = data.field; //获取提交的字段
                var index = parent.layer.getFrameIndex(window.name);
                $.ajax({
                    url: 'api/organization/employee-penalty/addEmployeePenalty',
                    type: 'POST',
                    data: field,
    ...
    exports('createEmployeePenalty', createEmployeePenalty);
});