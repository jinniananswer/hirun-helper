...
<head></head>
<body>
    <!-- 有 ALL_SHOP 操作权限则显示按钮 -->
    <button sec:authorize="hasAuthority('ALL_SHOP')" onclick="window.location.href='/admin'">选择全部门店</button>
    
    <!-- 有 ALL_SHOP 或 ALL_CITY 操作权限则显示按钮 -->
    <button sec:authorize="hasAnyAuthority('ALL_SHOP', 'ALL_CITY')" onclick="window.location.href='/admin'">选择全部门店</button>
</body>
</html>
```
